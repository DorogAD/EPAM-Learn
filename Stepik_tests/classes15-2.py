...
class Buffer:
    ...
    def add(self, *a):
        self.new_list = [*a]
        self.work_list = self.work_list + self.new_list
        self.rest = len(self.work_list) % 5
        while len(self.work_list) >= 5:
            five = self.work_list[0:5]
            result = five[0] + five[1] + five[2] + five[3] + five[4]
            print(result)
            self.work_list = self.work_list[5:]
        # добавить следующую часть последовательности

    def get_current_part(self):
        print_list = self.work_list[-self.rest:]
        print(print_list)# заменить на return!
