...
четырёх тестах, проверьте, что вы используете только русские символы для ответа.
В остальных случаях ищите ошибку в логике работы программы.
"""
n = input()


def do_choice_short(num):
    if num == 0 or num > 4:
...
def do_choice_long(num, last_num):
    one_num = int(last_num[-1])
    two_num = int(last_num)
    if 11 <= two_num <= 14 or one_num == 0 or 4 < one_num <= 9:
        print(str(num) + ' программистов')
    elif one_num == 1:
        print(str(num) + ' программист')
    else:
...
if len(n) == 1:
    do_choice_short(int(n))
else:
    do_choice_long(int(n), n[-2:])
