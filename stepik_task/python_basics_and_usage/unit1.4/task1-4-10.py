...
объявлена переменная a, но в пространстве global, внутри которого находится пространство foo, она объявлена.
...
space_dict = {'None': ['global']}
var_dict = {'None': ['None'], 'global': []}
testlist_1 = ['add global a', 'create foo global', 'add foo b', 'get foo a', 'get foo c', 'create bar foo', 'add bar a', 'get bar a', 'get bar b']
result = []



def create(namespace, parent):
    if parent not in space_dict:
        space_dict[parent] = [namespace]
    else:
        space_dict[parent].append(namespace)
    if namespace not in var_dict:
        var_dict[namespace] = []
...
def keyboard_input(a):
    for i in range(a):
        func, name, var = input().split()
        choice(func, name, var)


def test_input(testlist):
    for i in testlist:
        func, name, var = i.split()
        choice(func, name, var)


def choice(func, name, var):
    if func == 'create':
        create(name, var)
    elif func == 'add':
        add(name, var)
...
a = int(input())
if a <= 100:
    keyboard_input(int(a))
elif a > 100:
    test_input(testlist_1)
